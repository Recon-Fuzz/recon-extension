...
export class TemplateManager {
    ...
    private async updateGeneratedFile(filepath: string, newContent: string, marker: string): Promise<string> {
        try {
            const existingContent = await fs.readFile(filepath, 'utf8');

            // Check if file has the auto-generated section
            if (existingContent.includes(marker)) {
                const [beforeMarker, afterMarker] = existingContent.split(marker);

                // Extract the new auto-generated content
                const [_, newAutoGenerated] = newContent.split(marker);

                // Combine the existing content before the marker with the new auto-generated content
                return beforeMarker + marker + newAutoGenerated;
            }
    ...
    async generateTemplates(context: any = {}): Promise<void> {
        ...
        const canaries = otherFunctions.filter(fn => fn.mode === Mode.CANARY);
        ...
        const files: Record<string, string> = {
            ...
            [`${testFolder}/recon/CanaryStorage.sol`]: templates.canaryStorageTemplate({
                canaryVariables: canaries.map(fn => `${fn.abi.name}Canary`),
            }),
            [`${testFolder}/recon/Properties.sol`]: templates.propertiesTemplate({
                contracts: enabledContracts,
                canaryFunctions: canaries,
            }),
        ...
        for (const [name, content] of Object.entries(files)) {
            ...
            // Special handling for target function and properties files
            if (
                (name.includes('targets/') && name.endsWith('Targets.sol')) ||
                name.endsWith('Properties.sol')
            ) {
                const marker = name.endsWith('Properties.sol') ? 'CANARIES' : 'TARGET';
                const finalContent = await this.updateGeneratedFile(
                    outputPath,
                    content,
                    `/// AUTO GENERATED ${marker} FUNCTIONS - WARNING: DO NOT DELETE OR MODIFY THIS LINE ///`,
                );
